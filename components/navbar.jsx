...
const Navbar = () => {
  ...
  const menuItems = [
    ...
    { name: 'PRODUCTS', href: '/products', external: true }, // external link
    { name: '9 GRAHAS', href: '#grahas' },
    { name: 'भविष्य', href: '#bhavishya' },
    { name: 'FAQS', href: '#faqs' },
  ...
  return (
    <header className="sticky top-2 z-50 w-full bg-white shadow-sm px-6 py-4">
      <div className="max-w-7xl mx-auto flex items-center justify-between">
        ...
        {/* Nav Menu */}
        <nav className={`absolute md:static top-16 left-0 w-full md:w-auto bg-white md:bg-transparent z-40 transition-all duration-300 ease-in-out
            ...
            {menuItems.map((item, index) => (
              ...
              >
                {item.external ? (
                  ...
                  <a href={item.href} className="relative z-10" onClick={() => setIsOpen(false)}>
                    {item.name}
                  </a>
                )}
